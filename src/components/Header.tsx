...
export default function Header() {
    return (
            <header style={{
                ...
                <img src={'/img/global/index_header_sparky.svg'} className={'absolute left-[70%] -bottom-[30%] md:block hidden'}/>
                <div className={'mx-auto'}>
                    <div className={'container mx-auto pb-8 px-4 relative'}>
                        <Navbar/>
                        <div className={'py-8 md:text-left text-center'}>
                            <h1 className={'text-8xl text-black/75 font-bold font-display uppercase my-4'}>
                                Sticker für alle
                            </h1>
                            ...
                            <div className={'mt-6 flex flex-row gap-2 w-full md:justify-start justify-center'}>
                                {
                                    categories.filter(i => {
                                        ...
                                        <a href={'#'+category.id} target={'_self'} key={category.label+category.id}
                                           className={'bg-white/75 backdrop-blur-xs border border-white/75 hover:bg-white/85 rounded-full px-4 py-2'}>
                                            {category.label}
                                        </a>
                                    ))
                                }
                        ...
                        <img src={'/img/decoration/flower.svg'} className={'w-16 absolute md:top-[25%] top-[10%] left-[65%] animate-rotate'}/>
                    </div>
                </div>
            </header>
    )
